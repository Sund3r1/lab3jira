...
**Плохая практика:
**❌ Проблема: Секреты в открытом виде в логах. Доступны всем, кто видит логи.
```
echo "Hardcoded test credentials:"
echo "  Username: admin"
echo "  Password: Admin123!"
...
**Хорошая практика:
**✅ Исправление: Секреты хранятся в GitHub Secrets, не выводятся в логи.
```
env:
  TEST_MODE: ${{ secrets.TEST_MODE || 'development' }}
run: |
...
### 2. Обработка тестов
**Плохая практика:
**❌ Проблема: Игнорирование результатов тестов. Деплой продолжается даже при падении тестов.
```
if python -m pytest test_main.py -v; then
  echo "✅ Tests passed"
else
...
**Хорошая практика:
**✅ Исправление: Если тесты упадут, команда вернет ненулевой код выхода и workflow остановится.
```
run: |
  python -m pytest test_main.py -v
```
...
**Плохая практика:
**❌ Проблема: Установка с нуля каждый раз, без кеширования, медленно.
```
sudo apt-get update
sudo apt-get install -y python3 python3-pip python3-venv
```
**Хорошая практика:
**✅ Исправление: Использование готовых действий и кеширование.
```
- uses: actions/setup-python@v4
  with:
    python-version: "3.10"
...
**Плохая практика:
**❌ Проблема: Деплой всегда выполняется, нет условий.
```
- name: Deploy simulation
  run: |
    echo "🚀 STARTING DEPLOYMENT SIMULATION"
```
**Хорошая практика:
**✅ Исправление: Деплой только при успешном выполнении всех предыдущих шагов.
```
- name: Deploy simulation
  if: success()
  run: |
